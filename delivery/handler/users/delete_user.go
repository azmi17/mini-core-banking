...
func PermanentlyDeleteUser(ctx *gin.Context) {
	...
	if rerr != nil {
		...
		response := helper.ApiResponse("delete user failed", http.StatusUnprocessableEntity, "failed", errorMesage)
	...
	er := usecase.HardDeleteUser(payload.ListOfKodeLKM)
	if er != nil {
		if er == err.NoRecord {
